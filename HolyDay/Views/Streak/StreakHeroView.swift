...
struct StreakHeroView: View {
    ...
    private var accessibilityLabel: String {
        let dayLabel = streak.currentStreak == 1
            ? String(localized: "streak.day.vo")
            : String(localized: "streak.days.vo")
        return String(format: String(localized: "streak.accessibility.label"), streak.currentStreak, dayLabel)
    }

    private var accessibilityValue: String {
        let prayedThisWeek = daysOfCurrentWeek.filter { prayedDays.contains(calendar.startOfDay(for: $0)) }.count
        let todayDone = prayedDays.contains(today)
        let todayStatus = todayDone
