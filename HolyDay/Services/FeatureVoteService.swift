...
final class FeatureVoteService {
    ...
    func vote(for proposal: FeatureProposal) async {
        guard isConfigured,
              !proposal.hasVoted,
              let idx = proposals.firstIndex(where: { $0.id == proposal.id }) else { return }
        ...
        do {
            ...
            request.httpBody = try JSONEncoder().encode(["feature_id": proposal.id,
                                                         "device_id": deviceID])
    ...
    private func fetchFeatures() async throws -> [SupabaseFeature] {
        guard var components = URLComponents(string: "\(SupabaseConfig.url)/rest/v1/features_with_votes") else {
            throw URLError(.badURL)
        }
    ...
    private func fetchUserVotes() async throws -> [String] {
        ...
        components.queryItems = [URLQueryItem(name: "select", value: "feature_id"),
