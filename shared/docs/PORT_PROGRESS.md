...
- ✅ Rappels contextuels (`ReminderPlanner`, miroir de `ReminderPlanner.swift`, mêmes tests) — silence le jour où l'on a déjà prié, verset du thème de la dernière émotion pendant 2 jours, invitation « intentions » le dimanche si une intention ouverte a ≥ 7 jours (le lien `holyday://intentions` est posé ; sa prise en charge dans `HolyDayNavHost` arrive avec la gestion des deep links en cours). **Écart de mécanisme assumé** : iOS fige le contenu à la planification et replanifie après chaque prière ; Android lit Room au déclenchement dans le récepteur (`goAsync`). Règles et résultat perçu identiques.
